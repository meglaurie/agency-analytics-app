...
function Sidebar({selectedImage, deleteImage, handleLike, liked}) {
  const Heart = ({id}) => {
    return ( liked && selectedImage.favorited ? <svg id={id} width="15" height="13" viewBox="0 0 150 130" fill="red"  xmlns="http://www.w3.org/2000/svg">
      ...
      <div className={styles.container}>
        <div className={styles.imageContainer}>
          ...
          <div className={styles.imageText}>
            <div>
              <p style={{fontWeight: 'bold', marginBottom: '0px'}}>{selectedImage.filename}</p>
              <p style={{marginTop: '5px', color: '#64748b'}}>{((selectedImage.sizeInBytes/1024)/1000).toFixed(2) + "MB"}</p>
            </div>
            <div onClick={() => handleLike(selectedImage.favorited)} className={styles.heartBtn}>
